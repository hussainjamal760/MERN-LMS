...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("You are not enrolled to access this course", 404)
        );
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        userField instanceof mongoose.Types.ObjectId
      ) {
      ...
      if (!questionUser) {
        return next(
          new ErrorHandler("Question user not found in database", 500)
        );
      }
...
export const generateVideoUrl = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return next(
        new ErrorHandler(
          "Video OTP generation failed. Check server logs for details.",
          400
        )
      );
    }
